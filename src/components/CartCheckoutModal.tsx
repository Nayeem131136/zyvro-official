...
import { fetchAppSettings, formatPrice } from "@/lib/settings";
import { createCartOrder, buildWhatsappMessageMulti, whatsappUrl } from "@/lib/orders";
...
export function CartCheckoutModal({
  ...
  const [form, setForm] = useState({ name: "", phone: "", district: "", area: "", address: "", note: "" });

  if (!open) return null;

  const priceOf = (unitPrice: number) => (offer ? applySpinDiscount(unitPrice, offer.percent) : unitPrice);
  const itemsSubtotal = items.reduce((s, i) => s + priceOf(i.unitPrice) * i.quantity, 0);
  ...
  async function confirmOrder() {
    ...
    try {
      ...
      const { order, items: savedItems } = await createCartOrder({
        ...
        note: [
          ...
          offer ? `Spin Discount: -${offer.percent}%` : null,
          form.note.trim(),
        ].filter(Boolean).join(" — "),
        ...
        items: items.map((i) => ({
          ...
          product_name: i.name,
          product_url: `${window.location.origin}/product/${i.slug}`,
          color_name: i.colorName ?? undefined,
          size_name: i.sizeName ?? undefined,
          quantity: i.quantity,
          unit_price: priceOf(i.unitPrice),
        })),
      });

      const number = settings?.whatsapp_number || "8801577142710";
      const message = buildWhatsappMessageMulti(order, savedItems);
      ...
      clearCart();
      setStep(1);
      setForm({ name: "", phone: "", district: "", area: "", address: "", note: "" });
      setZone("dhaka");
      onSuccess();
    } catch (err) {
      console.error(err);
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-card border border-white/10 shadow-2xl">
        ...
        {step === 1 && (
          <form onSubmit={goToSummary} className="px-6 py-6 space-y-4">
            ...
            <Field label="Full Name *">
              <input required value={form.name} onChange={(e) => update("name", e.target.value)} className="input-zy" placeholder="Your name" />
            </Field>
            <Field label="Phone Number *">
              <input required value={form.phone} onChange={(e) => update("phone", e.target.value)} className="input-zy" placeholder="01XXXXXXXXX" />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="District *">
                <input required value={form.district} onChange={(e) => update("district", e.target.value)} className="input-zy" placeholder="Dhaka" />
              </Field>
              <Field label="Area *">
                <input required value={form.area} onChange={(e) => update("area", e.target.value)} className="input-zy" placeholder="Mirpur" />
              </Field>
            </div>
            <Field label="Full Address *">
              <textarea required rows={2} value={form.address} onChange={(e) => update("address", e.target.value)} className="input-zy resize-none" placeholder="House, road, area details" />
            </Field>
            <Field label="Order Note (optional)">
              <input value={form.note} onChange={(e) => update("note", e.target.value)} className="input-zy" placeholder="Any note" />
            </Field>
        ...
        {step === 2 && (
          <div className="px-6 py-6 space-y-5">
            <div className="border border-white/10 bg-black/30 p-4 space-y-3 text-sm">
              {items.map((it) => (
                <div key={it.key} className="flex justify-between gap-3">
                  <div className="min-w-0">
                    <div className="truncate">{it.name}</div>
                    <div className="text-[11px] text-muted-foreground">
                      {[it.colorName, it.sizeName, `× ${it.quantity}`].filter(Boolean).join("  |  ")}
                    </div>
                  </div>
                  <span className="text-[color:var(--gold-bright)] shrink-0">{formatPrice(priceOf(it.unitPrice) * it.quantity)}</span>
                </div>
              ))}
              <div className="h-px bg-white/10 my-2" />
              {offer && <div className="text-[11px] text-[color:var(--gold-bright)]">🎉 Spin Discount −{offer.percent}% applied to every item</div>}
              <SummaryRow label="Items Subtotal" value={formatPrice(itemsSubtotal)} />
            ...
            <div className="border border-[color:var(--gold)]/30 bg-[color:var(--gold)]/5 p-3 text-xs text-[color:var(--gold-bright)]">
              📌 To confirm this order, please pay only the delivery charge ({formatPrice(deliveryCharge)}) in advance via bKash/Nagad — product prices are Cash on Delivery. Our team will share payment details on WhatsApp.
            </div>

            <button onClick={confirmOrder} disabled={submitting} className="btn-zy w-full !py-3.5 disabled:opacity-60">
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : (<><MessageCircle className="h-4 w-4" /> Confirm & Send via WhatsApp</>)}
            </button>
